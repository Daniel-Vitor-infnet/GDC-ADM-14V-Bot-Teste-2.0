...
module.exports = {
  ...
  BloqueadoComando: function (comando, servidorDono) {
    const BloqueadoComando = new Discord.EmbedBuilder()
      .setColor(Cor.Permissao)
      .setAuthor({ name: 'Comando Bloqueado', iconURL: Gif.MiniGifAviso })
      .setDescription(`** O comando \`\`${comando}\`\` foi bloqueado neste servidor pelo dono <@${servidorDono}>. Qualquer dúvida entre em contato com a STAFF **`)
  ...
  BloqueadoComandoPadrao: function (comando, servidorDono) {
    const BloqueadoComandoPadrao = new Discord.EmbedBuilder()
      .setColor(Cor.Permissao)
      .setAuthor({ name: 'Comando Bloqueado', iconURL: Gif.MiniGifAviso })
      .setDescription(`** O comando \`\`${comando}\`\` é bloqueado por padrão será necessário que o dono <@${servidorDono}> desbloqueio. **`)
      .setThumbnail(Gif.ComandoBloqueado)
      .setFooter({ text: `Para mais dúvidas digite /ajuda`, iconURL: Gif.MiniGifHelp })
      .setTimestamp();
